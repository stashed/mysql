...
$ helm repo update
$ helm install stash-mysql-5.7.25-v2 appscode/stash-mysql -n kube-system --version=5.7.25-v2
```
...
To install the chart with the release name `stash-mysql-5.7.25-v2`:
...
To uninstall/delete the `stash-mysql-5.7.25-v2`:

```console
$ helm delete stash-mysql-5.7.25-v2 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore MySQL database. This is usually same as the database version it can backup. | `5.7.25-v2`         |
...
```console
$ helm install stash-mysql-5.7.25-v2 appscode/stash-mysql -n kube-system --version=5.7.25-v2 --set image.registry=stashed
```
...
```console
$ helm install stash-mysql-5.7.25-v2 appscode/stash-mysql -n kube-system --version=5.7.25-v2 --values values.yaml
```
