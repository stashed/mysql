...
$ helm repo update
$ helm install stash-mysql-8.0.3-rc.20200826 appscode/stash-mysql -n kube-system --version=8.0.3-rc.20200826
```
...
To install the chart with the release name `stash-mysql-8.0.3-rc.20200826`:
...
To uninstall/delete the `stash-mysql-8.0.3-rc.20200826`:

```console
$ helm delete stash-mysql-8.0.3-rc.20200826 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore MySQL database. This is usually same as the database version it can backup. | `8.0.3-rc.20200826` |
...
```console
$ helm install stash-mysql-8.0.3-rc.20200826 appscode/stash-mysql -n kube-system --version=8.0.3-rc.20200826 --set image.registry=stashed
```
...
```console
$ helm install stash-mysql-8.0.3-rc.20200826 appscode/stash-mysql -n kube-system --version=8.0.3-rc.20200826 --values values.yaml
```
